...
fn normalizes_https_url_to_full_id() {
    let cfg = parse_config(r#"plug "https://github.com/user/repo.git""#).unwrap();
    assert_eq!(cfg.plugins[0].remote_id().unwrap(), "github.com/user/repo");
}

#[test]
fn normalizes_https_url_without_git_suffix() {
    let cfg = parse_config(r#"plug "https://github.com/user/repo""#).unwrap();
    assert_eq!(cfg.plugins[0].remote_id().unwrap(), "github.com/user/repo");
}

#[test]
fn normalizes_ssh_git_url_to_full_id() {
    let cfg = parse_config(r#"plug "git@github.com:user/repo.git""#).unwrap();
    assert_eq!(cfg.plugins[0].remote_id().unwrap(), "github.com/user/repo");
}

#[test]
fn normalizes_gitlab_https_url() {
    let cfg = parse_config(r#"plug "https://gitlab.com/user/plugin.git""#).unwrap();
    assert_eq!(cfg.plugins[0].remote_id().unwrap(), "gitlab.com/user/plugin");
}

#[test]
fn normalizes_custom_host_url() {
    let cfg = parse_config(r#"plug "https://git.example.com/team/plugin.git""#).unwrap();
    assert_eq!(cfg.plugins[0].remote_id().unwrap(), "git.example.com/team/plugin");
}

#[test]
fn rejects_remote_ids_with_parent_segments() {
    let err = parse_config(r#"plug "https://git.example.com/team/../plugin.git""#).unwrap_err();
    assert!(err.to_string().contains("unsafe plugin id segment"), "{err}");
}

#[test]
fn rejects_remote_ids_with_empty_segments() {
    let err = parse_config(r#"plug "git@github.com:team//plugin.git""#).unwrap_err();
    assert!(err.to_string().contains("unsafe plugin id segment"), "{err}");
}
...
plug "https://github.com/tmux-plugins/tmux-sensible.git"
    "#;
...
fn local_plugin_has_no_remote_id() {
    let cfg = parse_config(r#"plug "~/dev/my-plugin" local=#true"#).unwrap();
